...
class Alidrive():
    ...
    def __init__(self):
        # aligo授权密钥
        if os.environ.get('ALIGO_TOKEN') is None:
            self.aligo = Aligo()
        else:
        ...
        def callable(path: str, file: BaseFile):
            ...
            if file.file_extension.lower() in ['mkv', 'mov', 'wmv', 'flv', 'avi', 'avchd', 'webm', 'mp4']:
                ...
                if flag:
                    ...
                    self.move_to_movies(file.parent_file_id,
                                        tmm_movies_folder.file_id, [])
            return None
    ...
    def move_to_movies(self, parent_file_id: str, tmm_file_id: str, path_list: list):
        ...
        if parent_file_id != tmm_file_id:
            ...
            for path in path_list:
                ...
                if path == path_list[-1]:
                    src_file: BaseFile | CreateFileResponse | None = self.get_folder_by_path(
                        file_path)
                    ...
                    for dp in final_path_dp:
                        ...
                        pass
                final_path += '/'+path
                final_path_dp.append(final_path.replace(
                    'movies', 'tmm/tmm-movies'))
